...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      const productionsData = await ProductionService.getAll();
  ...
  const hasAccess = (production: Production) => {
    ...
    // For now, allow access to all productions. In a real app, you'd check user permissions
    return true;
  };
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden">
      <nav className="nav-toolbar-modern">
        ...
        <div className="toolbar-group right">
          <div className="user-dropdown-modern">
            <span className="welcome-text-modern">Welcome, {user?.given_name || user?.email || 'User'}</span>
            <button 
              ref={avatarButtonRef}
            ...
            >
              <div className="user-avatar">
                {user?.given_name ? user.given_name.charAt(0).toUpperCase() : user?.email ? user.email.charAt(0).toUpperCase() : 'U'}
              </div>              
            </button>
          </div>
          <div className="ml-5"><ModeToggle /></div>
